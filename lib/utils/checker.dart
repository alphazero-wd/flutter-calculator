...
bool checkIsUnaryMinus(String expression, int index) {
  return index + 1 < expression.length && (checkIsDigit(expression[index + 1]) || expression[index + 1] == '(') && expression[index] == '-';
}
...
bool checkCannotPutOperator(String expression, String tokenToPut) {
  ...
  if (expression.isEmpty) {
    ...
    bool isNotUnaryMinus = tokenToPut != '-' && checkIsSpaceOrOpenParenthesis(expression[expression.length - 1]);
...
bool checkToAddWhiteSpace(String expression, String tokenToPut) {
  return expression.isNotEmpty &&
      ...
      (checkIsDigit(expression[expression.length - 1]) || checkIsDotOrPercentOrCloseParenthesis(expression[expression.length - 1]));
}
...
bool checkCannotAddDecimalPoint(String expression, String tokenToPut) {
  ...
  return _checkMultipleDecimalPointsInNumber(expression) || (checkIsDotOrPercentOrCloseParenthesis(expression[expression.length - 1]));
}

bool checkToAddZeroBeforeDecimalPoint(String expression, String tokenToPut) {
  return tokenToPut == '.' &&
      (expression.isEmpty || expression[expression.length - 1] == '-' || checkIsSpaceOrOpenParenthesis(expression[expression.length - 1]));
}
